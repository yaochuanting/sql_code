select 	case when t1.max_score_percentile >= 0.95 then 'A'
			 when t1.max_score_percentile >= 0.85 then 'B'
             when t1.max_score_percentile >= 0.6 then 'C'
             when t1.max_score_percentile >=0.4  then 'D'
		else 'E' end level,
...
from(
		select tpel.intention_id, ao.max_score_percentile, min(tpel.into_pool_date) as min_into_pool_date, s.submit_time
		...
		where tpel.into_pool_date >= '2020-02-01'
