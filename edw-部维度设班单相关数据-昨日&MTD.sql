select 
        t1.department_name, t1.manager, t1.number,
        ...
        if(t2.total_plan is null, 0, t2.total_plan) as today_plan,
...
from (
        select 
              cdme.department_name,tb.number,tb.manager,
        ...
        left join hf_mobdb.sales_tab tb on cdme.department_name=tb.group_name
        ...
        group by cdme.department_name, tb.number, tb.manager
        ) as t1
...
left join(
            select 
                   ...
                   count(distinct case when to_date(bb.max_date)='${analyse_date}' and bb.is_trial_deal = 1 then bb.student_intention_id end) as yes_trial_deal,
            ...
            from(
                    ...
                    left join (
                                ...
                                left join dt_mobdb.dt_charlie_dept_month_end cdme on cdme.user_id = tc.submit_user_id
                                          and cdme.stats_date = '${analyse_date}' and cdme.class = '销售'
                                          and to_date(cdme.`date`) >= trunc('${analyse_date}','MM')
                                where tcp.pay_status in (2, 4) and tc.status <> 8
                                group by tc.contract_id, tc.student_intention_id, cdme.department_name
