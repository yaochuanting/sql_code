select  date_sub(curdate(),interval 1 day) as stats_date,
        t1.department_name 部门, t2.order_number 订单数指标, t3.tomonth_order 当月完成订单数, t3.lastmonth_order 上月同期订单数,
        t3.tomonth_amount 当月完成订单额, t3.total_deal_saler 当月出单销售人数, t3.tomonth_rec_order 当月转介绍订单数, t4.new_rec_keys 当月转介绍线索量
...
left join (
            select a.department_name,
                   ...
                   count(distinct case when date(a.max_pay_date)>=date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01')
                                            and date(a.max_pay_date)<=date_sub(curdate(),interval 1 day)
            ...
            from(
                            ...
                            having real_pay_sum >= contract_amount 
                                   and date(max_pay_date) >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
