...
from( 
      select cdme.class, cdme.department, cdme.grp, cdme.department_name
      from bidata.charlie_dept_month_end cdme 
      where cdme.stats_date = curdate() and cdme.class = 'CR'
            and cdme.department_name like '%CR%'
      group by cdme.department_name
      ) as t1

left join (
            select a.department_name,
                   count(case when date(max_pay_date)=curdate() and a.new_sign=1 then a.contract_id end) as today_rec_order,
                   sum(case when date(max_pay_date)=curdate() and a.new_sign=1 then a.k_num end) as k_today_rec_order,
                   count(case when a.new_sign=1 then a.contract_id end) as tomonth_rec_order,
                   count(case when a.new_sign=1 then a.k_num end) as k_tomonth_rec_order,
                   sum(case when date(max_pay_date)=curdate() and a.new_sign=1 then a.contract_amount end) as today_rec_amount,
                   sum(case when a.new_sign=1 then a.contract_amount end) as tomonth_rec_amount,
            ...
            from(
                            ...
                            inner join bidata.charlie_dept_month_end cdme on cdme.user_id = tc.submit_user_id
                                       and cdme.stats_date = curdate() and cdme.class = 'CR'    
