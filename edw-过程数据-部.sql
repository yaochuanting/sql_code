select t1.department_name,
	   ...
	   t4.new_trial_exp,
	   concat(round(t4.new_trial_exp/t4.new_plan_num*100,2),'%') new_trial_exp_rate,
	   t4.new_trial_num,
	   ...
	   t2.all_oc_keys,
	   round(t7.all_oc_req/t6.staffs,2) ps_all_oc_keys,
	   t3.all_oc_apply_num,
	   concat(round(t3.all_oc_apply_num/t7.all_oc_req*100,2),'%') all_oc_req_apply_rate,
	   concat(round(t3.all_oc_apply_num/t2.all_oc_keys*100,2),'%') new_keys_apply_rate,
	   t4.all_oc_plan_num,
	   ...
	   t4.all_oc_trial_exp,
	   concat(round(t4.all_oc_trial_exp/t4.all_oc_plan_num*100,2),'%') all_oc_trial_exp_rate,
	   t4.all_oc_trial_num,
	   ...
	   concat(round(t5.all_oc_deal_num/t7.all_oc_req*100,2),'%') all_oc_req_deal_rate,
	   concat(round(t5.all_oc_deal_num/t2.all_oc_keys*100,2),'%') all_oc_keys_deal_rate,
	   ...
	   round(t4.new_rec_plan_num/t6.staffs,2) ps_new_rec_plan,
	   t4.new_rec_trial_exp,
	   concat(round(t4.new_rec_trial_exp/t4.new_rec_plan_num*100,2),'%') new_rec_trial_exp_rate,
	   ...
	   concat(round(t5.new_rec_deal_num/t2.new_rec_keys*100,2),'%') new_rec_keys_deal_rate
...
left join (
			...
			from(
					...
					where lp.lesson_type=2
						  ...
						  and lpo.apply_time<='${analyse_date}'
...
left join (
             ...
             from(    
					...
					left join dw_hf_mobdb.dw_lesson_relation lr on lpo.order_id=lr.order_id
					left join dw_hf_mobdb.dw_lesson_plan lp on lr.plan_id=lp.lesson_plan_id
					left join dw_hf_mobdb.dw_lesson_plan lp2 on lp2.student_id=lp.student_id and lp2.lesson_type=3 and lp2.status=3 and lp2.solve_status<>6
					left join dw_hf_mobdb.dw_view_student s on s.student_intention_id=lpo.student_intention_id
					inner join dt_mobdb.dt_charlie_dept_month_end cdme on cdme.user_id=lpo.apply_user_id 
							   and to_date(cdme.stats_date)='${analyse_date}' and cdme.class='CC'
							   and to_date(cdme.`date`)>=trunc('${analyse_date}','MM')
				    left join (
									...
									having to_date(max(tcp.pay_date))>=trunc('${analyse_date}','MM')
										   ...
										   and sum(tcp.sum/100)>=avg((tc.sum-666)*10)
...
left join (
             ...
             from(
					...
					group by tcp.contract_id, cdme.department_name
...
left join (
              select t.department_name, sum(t.number) as staffs  -- 抗标人数
              from(
		              select distinct cdme.department_name, st.number
...
left join (
             select cdme.department_name,
                    count(distinct case when to_date(s.create_time)>=trunc('${analyse_date}','MM') then ss.student_intention_id else null end) as new_req,  -- 新线索触碰量
	                count(distinct case when to_date(s.create_time)<trunc('${analyse_date}','MM') then ss.student_intention_id else null end) as all_oc_req  -- 大盘oc触碰量
             ...
             where to_date(ss.view_time)>=trunc('${analyse_date}','MM') 
                   and to_date(ss.view_time)<='${analyse_date}'
             group by department_name
