...
from(    
	    ...
	    left join (
						...
						group by tc.contract_id, tc.student_intention_id, concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
						having to_date(max(tcp.pay_date)) >= add_months(trunc('${analyse_date}','MM'),-1)
							   ...
							   and round(sum(tcp.sum/100)) >= round(avg((tc.sum-666)*10))
	                                      ...
	                                      and aa.region_name = concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
