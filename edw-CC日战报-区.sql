...
select   t1.region_name  -- 区名称
		...
		,t5.d_req  -- 昨日触碰总粒子数
		,t4.d_keys  -- 昨日获取总粒子数
		,t5.d_new_req  -- 昨日触碰新粒子数
		,t4.d_new_keys  -- 昨日获取新粒子数
		,t5.d_oc_req  -- 昨日触碰oc粒子数
		,t4.d_oc_keys  -- 昨日获取oc粒子数
		,t4.m_keys  -- 月粒子获取总数
		,t4.m_new_keys  -- 月新粒子获取数
		,t4.m_oc_keys  -- 月oc粒子获取数
		,coalesce(t6.d_tr_call_time,0) as d_tr_call_time  -- 昨日天润通时
		,coalesce(t7.d_wp_call_time,0) as d_wp_call_time  -- 昨日工作手机通时
		,coalesce(t6.d_tr_call_cnt,0) as d_tr_call_cnt  -- 昨日天润通次
		,coalesce(t7.d_wp_call_cnt,0) as d_wp_call_cnt  -- 昨日工作手机通次
		,coalesce(t6.m_tr_call_cnt,0) as m_tr_call_cnt  -- 月天润通次
		,coalesce(t7.m_wp_call_cnt,0) as m_wp_call_cnt  -- 月工作手机通次
		,coalesce(t6.m_tr_bridge_call_cnt,0) as m_tr_bridge_call_cnt  -- 月天润接通通次
		,coalesce(t7.m_wp_bridge_call_cnt,0) as m_wp_bridge_call_cnt  -- 月工作手机接通通次
		,coalesce(t6.m_tr_new_call_cnt,0) as m_tr_new_call_cnt  -- 月天润新粒子通次
		,coalesce(t6.m_tr_new_bridge_call_cnt,0) as m_tr_new_bridge_call_cnt  -- 月天润新粒子接通通次
		,coalesce(t6.m_tr_oc_call_cnt,0) as m_tr_oc_call_cnt  -- 月天润oc粒子通次
		,coalesce(t6.m_tr_oc_bridge_call_cnt,0) as m_tr_oc_bridge_call_cnt  -- 月天润oc粒子接通通次
		,t8.m_commu_cnt  -- 月总粒子沟通频次
		,t8.m_new_commu_cnt  -- 月新粒子沟通频次
		,t8.m_oc_commu_cnt  -- 月oc粒子沟通频次
		,t9.d_apply_num  -- 昨日发起设班单数
		,t10.d_exp  -- 昨日体验课数
		,t11.d_plan_num  -- 昨日试听课排课数
		,t11.d_trial_num  -- 昨日试听课出席数
		,t9.m_apply_num  -- 月发起设班单数
		,t10.m_exp  -- 月体验课数
		,t11.m_plan_num  -- 月试听课排课数
		,t11.m_trial_num  -- 月试听出席数
		,t12.d_trial_deal  -- 昨日试听关单量
		,t12.rc7_trial_deal  -- 近七日试听关单量
		,t12.rc7_trial_num  -- 近七日试听出席量
		,t12.rc30_trial_deal  -- 近30日试听关单量
		,t12.rc30_trial_num  -- 近30日试听出席量
		,t13.m_order  -- 月总成单数
		,t13.m_new_order  -- 月新粒子成单数
		,t13.m_oc_order  -- 月oc粒子成单数
		,t11.m_hourdiff  -- 月试听关单总时长
		,t11.m_hourdiff_notnull  -- 月支付大于关单的试听成单量
		,t13.d_order  -- 昨日订单数
		,t13.d_new_order  -- 昨日新粒子订单数
		,t13.d_oc_order  -- 昨日oc粒子订单数
		,t13.m_rec_order  -- 当月转介绍订单数
		,t2.order_number  -- 当月订单数指标
		,t13.m_order_amount  -- 当月总订单额
		,t13.d_order_amount  -- 昨日订单额
		,t12.d_rec_order  -- 昨日转介绍订单数
		,t11.m_plan_exp  -- 月邀约且出席体验课量


from (
		-- 销售部门架构
		select distinct concat(coalesce(cdme.center,''),coalesce(cdme.region,'')) as region_name
...
left join (
				select	a.region_name
						,sum(a.number) number
						,sum(a.order_number) order_number	
				from(
							select  distinct concat(coalesce(cdme.center,''),coalesce(cdme.region,'')) region_name
								    ,st.group_name
								    ,st.number
								    ,st.order_number
							from hf_mobdb.sales_tab st
							left join dt_mobdb.dt_charlie_dept_month_end cdme on cdme.department_name = st.group_name
...
left join (
				...
				from(
				            select  concat(coalesce(cdme.center,''),coalesce(cdme.region,'')) region_name
						            ,cdme.user_id
				            ...
				            group by concat(coalesce(cdme.center,''),coalesce(cdme.region,'')), cdme.user_id
				            ) as a
				group by a.region_name
...
left join (
				...
				group by concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
				) as t8 on t8.region_name = t1.region_name
...
left join (
				...
				group by concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
				) as t9 on t9.region_name = t1.region_name
...
left join (
				...
				group by concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
				) as t10 on t10.region_name = t1.region_name
...
left join (
			    ...
			    left join (
			    				select max(tcp.pay_date) max_date,
				                    ...
				                    group by tc.contract_id, concat(coalesce(cdme.center,''),coalesce(cdme.region,'')), tc.student_intention_id, tcp.submit_user_id
				                    having max(tcp.pay_date)>=date_sub('${analyse_date}',30)
				                           ...
				                           and round(sum(tcp.`sum`/100),0)>=round(max((tc.`sum`-666)*10),0)
				                                     ...
				                                     and a.region_name=concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
				...
				where lp.lesson_type=2
					  ...
					  and s.account_type = 1
				group by concat(coalesce(cdme.center,''),coalesce(cdme.region,''))
				) as t11 on t11.region_name = t1.region_name
...
left join (
				...
				from(
				      ...
				      left join (
				                    ...
				                    inner join dt_mobdb.dt_charlie_dept_month_end cdme on cdme.user_id = tcp.submit_user_id
				                           ...
				                           and to_date(cdme.`date`)>=date_sub('${analyse_date}',30)
				                    where tcp.pay_status in (2,4) and tc.status<>8
				                    group by tc.contract_id, concat(coalesce(cdme.center,''),coalesce(cdme.region,'')), 
				...
				) as t12 on t12.region_name = t1.region_name
...
left join (
				...
				) as t13 on t13.region_name = t1.region_name