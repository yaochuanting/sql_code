select x.stats_date 统计日期,
       x.department_name 部门,
       x.region 区,
       x.department 部,
	   xy.staffs 月初抗标人数,
	   xz.new_req 新粒子触碰量,
	   y.new_keys 新粒子实获量,
     aa.last_month_new_keys 上月同期获取新粒子量,
	   yx.new_apply_num 新粒子申请设班单量,
	   z.new_plan_num 新粒子试听邀约量,
	   z.new_trial_exp 新粒子体验课出席量,
	   z.new_trial_num 新粒子试听出席量,
	   z.new_trial_deal 新粒子试听成单量,
     bb.last_month_new_plan_num 上月同期新线索试听邀约量,
     bb.last_month_new_trial_num 上月同期新线索试听出席量,
     bb.last_month_new_trial_deal 上月同期新线索试听成单量,
	   xx.new_deal_num 新粒子总成单量,
	   xx.new_deal_amount 新粒子总成单额,
	   ...
	   y.new_rec_keys 获取转介绍粒子量,
	   z.new_rec_plan_num 新转介绍粒子试听邀约量,
...
left join (
             select 
	                  ...
	                  count(distinct intention_id) all_keys, -- 获取的总粒子量
                      count(distinct case when s.submit_time >= date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01')
			                              then intention_id end) new_keys, -- 获取的新粒子量
	                  count(distinct case when s.submit_time < date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01')
			                              then intention_id end) all_oc_keys,  -- 获取的大盘oc粒子量
	                  count(distinct case when s.submit_time >= date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01')
			                                   and (s.coil_in in (13,22) or s.know_origin in (56,71,22,24,25,41))
						                  then intention_id end) new_rec_keys	 -- 获取新转介绍新粒子量										 
             from
                   (
			 ...
			 ) as y on y.department_name = x.department_name and y.stats_date = x.stats_date
...
left join (
			...
			group by stats_date, bb.department_name
			) as yx on yx.stats_date = x.stats_date
                       and yx.department_name = x.department_name
...
left join (
             select
                      ...
                      count(distinct(case when b.key_attr = 'new' then b.student_intention_id else null end)) new_plan_num, -- 新粒子试听邀约量
		              count(distinct(case when b.key_attr = 'all_oc' then b.student_intention_id else null end)) all_oc_plan_num, -- 大盘oc粒子试听邀约量,
                      count(distinct(case when b.is_recommend = 1 and b.key_attr = 'new' then b.student_intention_id else null end)) new_rec_plan_num, -- 转介绍粒子试听邀约量
		              count(distinct(case when b.is_trial = 1 then b.student_intention_id else null end)) all_trial_num, -- 总试听出席量
		              count(distinct(case when b.key_attr = 'new' and b.is_trial = 1 then b.student_intention_id else null end)) new_trial_num, -- 新粒子试听出席量
                      ...
                      count(distinct(case when b.key_attr = 'new' and b.is_recommend = 1 and b.is_trial = 1 then b.student_intention_id else null end)) new_rec_trial_num, -- 转介绍新粒子试听出席量
	                  count(distinct(case when b.is_trial_deal = 1 then b.student_intention_id else null end)) all_trial_deal, -- 总试听成单量
		              count(distinct(case when b.key_attr = 'new' and b.is_trial_deal = 1 then b.student_intention_id else null end)) new_trial_deal, -- 新粒子试听成单量
		              count(distinct(case when b.key_attr = 'all_oc' and b.is_trial_deal = 1 then b.student_intention_id else null end)) all_oc_trial_deal, -- 大盘oc粒子试听成单量
		              count(distinct(case when b.key_attr = 'new' and b.is_trial_deal = 1 and b.is_recommend = 1 then b.student_intention_id else null end)) new_rec_trial_deal, -- 转介绍新粒子试听成单量
		              count(distinct(case when b.is_trial_exp = 1 then b.student_intention_id else null end)) all_trial_exp, -- 总体验课数量
		              count(distinct(case when b.key_attr = 'new' and b.is_trial_exp = 1 then b.student_intention_id else null end)) new_trial_exp, -- 新粒子体验课数量
		              count(distinct(case when b.key_attr = 'all_oc' and b.is_trial_exp = 1 then b.student_intention_id else null end)) all_oc_trial_exp, -- 大盘oc粒子体验课数量
		              count(distinct(case when b.key_attr = 'new' and b.is_trial_exp = 1 and b.is_recommend = 1 then b.student_intention_id else null end)) new_rec_trial_exp -- 转介绍新粒子体验课数量
             ...
             group by department_name, stats_date
			 ) as z on z.department_name = x.department_name and z.stats_date = x.stats_date
...
left join (
             select  
					...
					count(case when a.key_attr = 'new' then a.contract_id else null end) as new_deal_num,   -- 新粒子成单量
					sum(case when a.key_attr = 'new' then a.real_pay_sum else 0 end) as new_deal_amount,   -- 新粒子成单额
					count(case when a.key_attr = 'all_oc' then a.contract_id else null end) as all_oc_deal_num,   -- 大盘oc粒子成单量
					sum(case when a.key_attr = 'all_oc' then a.real_pay_sum else 0 end) as all_oc_deal_amount,   -- 大盘oc粒子成单额
					count(case when a.key_attr = 'new' and (a.coil_in in (13,22) or a.know_origin in (56,71,22,24,25,41))
							   then a.contract_id else null end) as new_rec_deal_num,   -- 新粒子转介绍成单量
					sum(case when a.key_attr = 'new' and (a.coil_in in (13,22) or a.know_origin in (56,71,22,24,25,41))
							 then a.real_pay_sum else 0 end) as new_rec_deal_amount   -- 新粒子转介绍成单额
             ...
             group by a.department_name, stats_date
			 ) as xx on xx.department_name = x.department_name and xx.stats_date = x.stats_date
...
left join (
              ...
              from(
               ...
               group by department_name,stats_date
			   ) as xy on xy.department_name = x.department_name and xy.stats_date = x.stats_date
...
left join (
             ...
             group by department_name, stats_date
			 ) as xz on xz.department_name = x.department_name and xz.stats_date = x.stats_date
...
left join (
             select 
                    date_sub(curdate(),interval 1 day) stats_date,
                    cdme.department_name,
                    count(distinct case when s.submit_time >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
                                        and s.submit_time< date_sub(curdate(),interval 1 month)
			                              then intention_id end) last_month_new_keys
             ...
             from
                   (
                       ...
                       inner join bidata.charlie_dept_month_end cdme on cdme.user_id = tpel.track_userid
	                              and cdme.stats_date = curdate() and cdme.class = 'CC'
                       left join hfjydb.view_user_info ui on ui.user_id = tpel.create_userid
                       where tpel.into_pool_date >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
	                         and tpel.into_pool_date < date_sub(curdate(),interval 1 month)
                       union all
                       select tnn.user_id as track_userid, tnn.student_intention_id as intention_id, tnn.create_time as into_pool_date, 'OC分配账号' distri_user
                       from hfjydb.tms_new_name_get_log tnn
                       inner join bidata.charlie_dept_month_end cdme on cdme.user_id = tnn.user_id
	                              and cdme.stats_date = curdate() and cdme.class = 'CC'
                       where tnn.create_time >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
	                         and tnn.create_time < date_sub(curdate(),interval 1 month)
	                         and student_intention_id <> 0
							     ) as a							 
             ...
             group by department_name, stats_date
			 ) as aa on aa.department_name = x.department_name and aa.stats_date = x.stats_date

-- 上月同期新粒子试听邀约量、出席量及试听成单量
left join (
             select
                  	  date_sub(curdate(),interval 1 day) stats_date,
		              b.department_name,
                      count(distinct(case when b.key_attr = 'new' then b.student_intention_id else null end)) last_month_new_plan_num, 
		              count(distinct(case when b.key_attr = 'new' and b.is_trial = 1 then b.student_intention_id else null end)) last_month_new_trial_num,
		              count(distinct(case when b.key_attr = 'new' and b.is_trial_deal = 1 then b.student_intention_id else null end)) last_month_new_trial_deal
             from(    
		                select
									   lpo.apply_user_id, lpo.student_intention_id, s.student_no,
									   cdme.department_name,
									   (case when lp.status in (3,5) and lp.solve_status <> 6 then 1 else 0 end) is_trial,
									   (case when s.submit_time >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
		                            			  and s.submit_time < date_sub(curdate(),interval 1 month)
					    ...
					    left join (
										...
										having max(tcp.pay_date) >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
											   and max(tcp.pay_date) < date_sub(curdate(),interval 1 month)
											   and real_pay_amount >= contract_amount
											   ) as aa on aa.student_intention_id = lpo.student_intention_id
														  and aa.department_name = cdme.department_name			   
             where lp.lesson_type = 2
				   and lp.adjust_start_time >= date_sub(date_format(date_sub(curdate(),interval 1 day),'%Y-%m-01'),interval 1 month)
				   and lp.adjust_start_time < date_sub(curdate(),interval 1 month)
				   and s.account_type = 1
				   ) as b
             group by department_name, stats_date
			 ) as bb on bb.department_name = x.department_name and bb.stats_date = x.stats_date