-- CC日战报-部门
select   t1.department_name  -- 部门名称
		,t1.user_id  -- 销售id
		,t1.name  -- 销售姓名
		,t1.job_number  -- 员编
		,t1.min_opt_time  -- 入职时间
		,t3.d_req  -- 昨日触碰总粒子数
		,t2.d_keys  -- 昨日获取总粒子数
		,t3.d_new_req  -- 昨日触碰新粒子数
		,t2.d_new_keys  -- 昨日获取新粒子数
		,t3.d_oc_req  -- 昨日触碰oc粒子数
		,t2.d_oc_keys  -- 昨日获取oc粒子数
		,t2.m_keys  -- 月粒子获取总数
		,t2.m_new_keys  -- 月新粒子获取数
		,t2.m_oc_keys  -- 月oc粒子获取数
		,coalesce(t4.d_tr_call_time,0) as d_tr_call_time  -- 昨日天润通时
		,coalesce(t5.d_wp_call_time,0) as d_wp_call_time  -- 昨日工作手机通时
		,coalesce(t4.d_tr_call_cnt,0) as d_tr_call_cnt  -- 昨日天润通次
		,coalesce(t5.d_wp_call_cnt,0) as d_wp_call_cnt  -- 昨日工作手机通次
		,coalesce(t4.m_tr_call_cnt,0) as m_tr_call_cnt  -- 月天润通次
		,coalesce(t5.m_wp_call_cnt,0) as m_wp_call_cnt  -- 月工作手机通次
		,coalesce(t4.m_tr_bridge_call_cnt,0) as m_tr_bridge_call_cnt  -- 月天润接通通次
		,coalesce(t5.m_wp_bridge_call_cnt,0) as m_wp_bridge_call_cnt  -- 月工作手机接通通次
		,coalesce(t4.m_tr_new_call_cnt,0) as m_tr_new_call_cnt  -- 月天润新粒子通次
		,coalesce(t4.m_tr_new_bridge_call_cnt,0) as m_tr_new_bridge_call_cnt  -- 月天润新粒子接通通次
		,coalesce(t4.m_tr_oc_call_cnt,0) as m_tr_oc_call_cnt  -- 月天润oc粒子通次
		,coalesce(t4.m_tr_oc_bridge_call_cnt,0) as m_tr_oc_bridge_call_cnt  -- 月天润oc粒子接通通次
		,t6.m_commu_cnt  -- 月总粒子沟通频次
		,t6.m_new_commu_cnt  -- 月新粒子沟通频次
		,t6.m_oc_commu_cnt  -- 月oc粒子沟通频次
		,t7.d_apply_num  -- 昨日发起设班单数
		,t8.d_exp  -- 昨日体验课数
		,t9.d_plan_num  -- 昨日试听课排课数
		,t9.d_trial_num  -- 昨日试听课出席数
		,t7.m_apply_num  -- 月发起设班单数
		,t8.m_exp  -- 月体验课数
		,t9.m_plan_num  -- 月试听课排课数
		,t9.m_trial_num  -- 月试听出席数
		,t10.d_trial_deal  -- 昨日试听关单量
		,t10.rc7_trial_deal  -- 近七日试听关单量
		,t10.rc7_trial_num  -- 近七日试听出席量
		,t10.rc30_trial_deal  -- 近30日试听关单量
		,t10.rc30_trial_num  -- 近30日试听出席量
		,t11.m_order  -- 月总成单数
		,t11.m_new_order  -- 月新粒子成单数
		,t11.m_oc_order  -- 月oc粒子成单数
		,t9.m_hourdiff  -- 月试听关单总时长
		,t9.m_hourdiff_notnull  -- 月支付大于关单的试听成单量
		,t11.d_order  -- 昨日订单数
		,t11.d_new_order  -- 昨日新粒子订单数
		,t11.d_oc_order  -- 昨日oc粒子订单数
		,t11.m_rec_order  -- 当月转介绍订单数
		,t2.order_number  -- 当月订单数指标
		,t11.m_order_amount  -- 当月总订单额
		,t11.d_order_amount  -- 昨日订单额
		,t11.d_rec_order  -- 昨日转介绍订单数
		,t9.m_plan_exp  -- 月邀约且出席体验课量


from (
		-- 销售部门架构
		select cdme.user_id, cdme.name, cdme.job_number, cdme.department_name, to_date(min(opt_time)) min_opt_time
		...
		group by cdme.user_id, cdme.job_number, cdme.department_name
		) as t1
...
left join (
			select  ss.user_id
             ...
             group by ss.user_id
             ) as t3 on t3.user_id = t1.user_id
...
left join (
			...
			group by pr.user_id
			) as t5 on t5.user_id = t1.user_id


-- 沟通频次
left join (
				select  cr.communication_person
				...
				group by cr.communication_person
				) as t6 on t6.communication_person = t1.user_id
...
left join (
				...
				group by lpo.apply_user_id
				) as t7 on t7.apply_user_id = t1.user_id
...
left join (
				...
				) as t8 on t8.user_id = t1.user_id
...
left join (
			    				...
			    				select max(tcp.pay_date) max_date,
				                    ...
				                    having max(tcp.pay_date)>=date_sub('${analyse_date}',30)
				                           ...
				                           and round(sum(tcp.`sum`/100),0)>=round(max((tc.`sum`-666)*10),0)
				                                     ...
				                                     and a.submit_user_id = lpo.apply_user_id
				...
				where lp.lesson_type=2
					  ...
					  and s.account_type = 1
				group by lpo.apply_user_id
				) as t9 on t9.apply_user_id = t1.user_id



-- 试听关单率
left join (
				select b.apply_user_id
				...
				from(
				      select lpo.student_intention_id, lpo.apply_user_id, lp.adjust_start_time, a.max_date, a.contract_id
				      from dw_hf_mobdb.dw_lesson_plan_order lpo
				      left join dw_hf_mobdb.dw_lesson_relation lr on lpo.order_id = lr.order_id
				      left join dw_hf_mobdb.dw_lesson_plan lp on lr.plan_id = lp.lesson_plan_id
				      left join dw_hf_mobdb.dw_view_student s on s.student_intention_id = lpo.student_intention_id
				...
				group by b.apply_user_id
				) as t10 on t10.apply_user_id = t1.user_id


-- 总成单数成单额
left join (
				select  d.submit_user_id
				...
				group by d.submit_user_id
				) as t11 on t11.submit_user_id = t1.user_id
