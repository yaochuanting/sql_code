...
select a.department_name,
       count(case when date(max_pay_date)='${analyse_date}' then a.contract_id end) as today_order,
       sum(case when date(max_pay_date)='${analyse_date}' then a.k_order_num end) k_today_order,
...
from(
        select  
                    ...
                    collect_set(case when s.coil_in in (13,22) or s.know_origin in (56,71,22,24,25,41) then 1 else 0 end) as is_rec,
                    collect_set(case when s.create_time>=trunc('${analyse_date}','MM') then 1 else 0 end) as is_new,
                    collect_set(case when tc.period>=60 then 1 else 0.5 end) as k_order_num, 
                    max(tcp.pay_date) as max_pay_date,
                    sum(tcp.`sum`/100) real_pay_sum, 
                    collect_set((tc.`sum`-666)*10) contract_amount
        ...
        having round(sum(tcp.`sum`/100),0)>=round((tc.`sum`-666)*10,0)
