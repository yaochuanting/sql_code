...
left join (
				...
				) as t2 on t1.department_name = t2.group_name
...
left join (
            ...
            from
                   (
                       select tpel.track_userid, tpel.intention_id, tpel.into_pool_date, cdme.department_name
                       ...
                       union
                       select tnn.user_id as track_userid, tnn.student_intention_id as intention_id, tnn.create_time as into_pool_date, cdme.department_name
                              ,case when to_date(s.create_time)>=trunc('${analyse_date}','MM') then 1 else 0 end is_new
                              ,case when s.coil_in in (13,22) or s.know_origin in (56,71,22,24,25,41) then 1 else 0 end is_rec
...
left join (
			...
			group by cdme.department_name
			) as t7 on t7.department_name = t1.department_name
...
left join (
				...
				group by cdme.department_name
				) as t8 on t8.department_name = t1.department_name
...
left join (
				...
				group by cdme.department_name
				) as t9 on t9.department_name = t1.department_name
...
left join (
				...
				group by cdme.department_name
				) as t10 on t10.department_name = t1.department_name
...
left join (
				...
				where lp.lesson_type=2
					  ...
					  and s.account_type = 1
				group by cdme.department_name
				) as t11 on t11.department_name = t1.department_name
...
left join (
				...
				) as t12 on t12.department_name = t1.department_name
...
left join (
				...
				) as t13 on t13.department_name = t1.department_name
